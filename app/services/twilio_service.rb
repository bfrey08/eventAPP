...
class TwilioService
  ...
  def self.create_message(attendees)
    ...
    attendees.each do |attendee|
      body = "you are invited to '#{attendee.event.name}' by #{attendee.event.attendees.first.name}, join us: http://localhost:3000/events/#{attendee.event.id}/attendees/#{attendee.id}/login/#{attendee.api_key}"
